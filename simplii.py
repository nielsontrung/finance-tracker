...
def get_ofx_transaction():
    ...
    input_file = json.loads(config).get("simplii").get("filepath")
    df = pd.read_csv(
        input_file,
...
def get_year_summary_simplii():
    config = json.loads(open("./config.json").read()).get("simplii")
...
def get_transactions_simplii():
    config = json.loads(open("./config.json").read()).get("simplii")
    ofx_transactions = get_ofx_transaction()
    year_summary_transactions = get_year_summary_simplii()
    ...
    output_file = f"{output_folder}\\simplii_all_transactions.csv"
    print(f'{datetime.now()} saving all simplii transactions: "{output_file}"')
    df.to_csv(output_file, index=False)
    df.to_csv("./csv/simplii_all_transactions.csv", index=False)
    return transactions


# get_ofx_transaction()
# get_year_summary_simplii()
# get_transactions_simplii()
